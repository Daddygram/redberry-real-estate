...
interface ListingProps {
    listing: Inputs; // Use the `Inputs` type for listing
  }
    ...
    <Link to={`/listing/${listing.id}`} className="flex flex-col h-full rounded-[14px] cursor-pointer overflow-hidden hover:shadow-custom">
        ...
        <div className="relative">
            <span className="absolute top-[23px] left-[23px] px-[26.5px] py-[8.5px] bg-black/50 rounded-[20px]">
                <p className="-translate-y-[1px] text-white font-medium text-xs leading-[0.9rem]">{listing.is_rental ? 'ქირავდება' : 'იყიდება'}</p>
            </span>
            <img src={listing.image} alt="" className="w-auto h-[307px] object-cover rounded-t-[14px]" />
        </div>

        {/* listing details */}
        <div className="px-[25px] py-[22px] border border-x border-b border-grey rounded-b-[14px]">
            {/* price */}
            <span className="text-black font-bold text-[1.75rem] leading-[2.1rem]">{formattedPrice} ₾</span>
            {/* location */}
            ...
            <div className="mt-5 flex items-center gap-8">
                ...
                <div className="flex justify-center items-center gap-2">
                    ...
                    <p className="text-black/70 leading-[1.2rem] ">{listing.bedrooms}</p>
                </div>
                {/* area */}
                <div className="flex justify-center items-center gap-2">
                    ...
                    <p className="text-black/70 leading-[1.2rem] ">{listing.area} მ<span className="mb-[7px] text-[0.625rem] leading-[0.75rem]">2</span></p>
                </div>
                {/* postal code */}
                <div className="flex justify-center items-center gap-2">
                    ...
                    <p className="text-black/70 leading-[1.2rem] ">{listing.zip_code}</p>
                </div>
            </div>
        </div>
    </Link>
